...
namespace Com.Efrata.Service.Purchasing.Lib.PDFTemplates
{
    public class GarmentExternalPurchaseOrderPDFTemplate
    {
        public MemoryStream GeneratePdfTemplate(GarmentExternalPurchaseOrderViewModel viewModel, int clientTimeZoneOffset)
        {
            ...
            cellHeaderContentLeft.Phrase = new Phrase("PT Efrata GARMINDO" +  "\n" + "Banaran, Grogol, Sukoharjo" + "\n" + "Jawa Tengah 57552 - INDONESIA" + "\n" + "Telp. (0271) 732888, 7652913", bold_font);
            ...
            if (viewModel.Supplier.Import)
            {
                string p1 = "The undersigned below, PT. Efrata Garmindo, SOLO (hereinafter referred to as parties Purchasers) and " + viewModel.Supplier.Name + " (hereinafter referred to as seller\'s side) mutually agreed to enter into a sale and purchase contract with the following conditions: ";
            ...
            {
                string p1 = "Dengan hormat,\nYang bertanda tangan di bawah ini, PT. Efrata Garmindo, SOLO (selanjutnya disebut sebagai pihak Pembeli) dan " + viewModel.Supplier.Name + " (selanjutnya disebut sebagai pihak Penjual) saling menyetujui untuk mengadakan kontrak jual beli dengan ketentuan sebagai berikut: ";
                Paragraph firstParagraph = new Paragraph(p1, normal_font) { Alignment = Element.ALIGN_LEFT };
